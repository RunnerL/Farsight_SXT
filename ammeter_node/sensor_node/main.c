...
void get_ammeter_msgs(uint16_t *usRegBuf, uint8_t *lock_cmd);

void get_sensor_data(void)
{
  send_data(USART2,ammeter_command,AMT_CMD_SIZE);
}

void get_ammeter_msgs(uint16_t *usRegBuf, uint8_t *amt_cmd)
{
  usRegBuf[0] = ((amt_cmd[16] - 0x33) << 8) + (amt_cmd[15] - 0x33);//数据高位
  usRegBuf[1] = ((amt_cmd[14] - 0x33) << 8) + (amt_cmd[13] - 0x33);//数据地位
...
int main(void)
{
  ...
  while (1) {
    ...
    if (ammeter_fifo.count >= 18) {
      checksum = 0;
      for(num=1;num<17;num++)
      {
        checksum += ammeter_fifo.dat[num];
      }
      //QueueInit(&ammeter_fifo);
 
      if(checksum == ammeter_fifo.dat[17]){
      get_ammeter_msgs(usRegHoldingBuf,ammeter_fifo.dat);
#if 1
      printf("V: %02x %02x %02x . %02x kwh\n",(usRegHoldingBuf[0] >> 8),(usRegHoldingBuf[0] && 0x00ff),(usRegHoldingBuf[1] >> 8),(usRegHoldingBuf[1] && 0x00ff));
#endif 
     // Delay(10);
      QueueInit(&ammeter_fifo);
      }
...
void Usart_Init(USART_TypeDef* USARTx,uint32_t ulBaudRate)
{
  ...
  USART_InitStructure.USART_Parity = USART_Parity_Even;//USART_Parity_No,USART_Parity_Even,IS_USART_PARITY
